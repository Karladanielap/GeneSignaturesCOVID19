...
def laplacian(W, normalized=True, renormalized=True):
    ...
    if not normalized:
        ...
        Wr = W
        dr = Wr.sum(axis=0)
        dr += np.spacing(np.array(0, W.dtype))
        dr = 1 / np.sqrt(d)
        Dr = scipy.sparse.diags(d.A.squeeze(), 0) #scipy.sparse.diags(d.A.squeeze(), 0)
        L = Dr * Wr * Dr
    else:    
        d += np.spacing(np.array(0, W.dtype))
        d = 1 / np.sqrt(d)
