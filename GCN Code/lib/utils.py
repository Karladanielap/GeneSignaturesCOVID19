...
def grid_search(params, grid_params, train_data, train_labels, val_data,
    ...
    param_list = list(
        sklearn.model_selection.ParameterSampler(grid_params, n_iter=500, random_state=np.random.RandomState()))
    for grid_params in param_list:
        print(grid_params)
    print('Random search: {} out of {} combinations to evaluate'.format(len(param_list), len(grid)))
    for grid_params in param_list:
        params.update(grid_params)
        name = '{}'.format(param_list)
        print('\n\n  {}/{} {}  \n\n'.format(count, len(param_list), grid_params))
        count += 1
        m = model(params)
        m.fit(train_data, train_labels, val_data, val_labels)
        string, accuracy, f1, loss = m.evaluate(train_data, train_labels, Name='Train')
        train_accuracy.append('{:5.2f}'.format(accuracy))
        train_f1.append('{:5.2f}'.format(f1))
        print('train {}'.format(string))
        string, accuracy, f1, loss = m.evaluate(test_data, test_labels, Name='Test')
    ...
    print('Train F1 (weighted): {}'.format(' '.join(train_f1)))
    print('Test F1 (weighted):  {}'.format(' '.join(test_f1)))
    for i, grid_params in enumerate(param_list):
        print('{} --> train_accuracy: {} test_accuracy: {} train_f1: {} test_f1: {}'
              .format(grid_params, train_accuracy[i], test_accuracy[i], train_f1[i], test_f1[i]))
...
class model_perf(object):
    ...
    def __init__(s):
        ...
        s.fit_accuracies, s.fit_losses, s.fit_time, s.train_accuracies, s.train_losses, s.epoch = {}, {}, {}, {}, {}, {}
        s.train_accuracy, s.train_f1, s.train_loss = {}, {}, {}
        s.test_accuracy, s.test_f1, s.test_loss = {}, {}, {}
        s.val_accuracy, s.val_f1, s.val_loss = {}, {}, {}

    def test(s, model, name, params, train_data, train_labels, val_data, val_labels, test_data, test_labels, KD=False):
        s.params[name] = params
        s.fit_accuracies[name], s.fit_losses[name], s.fit_time[name], s.train_accuracies[name], s.train_losses[name], \
        s.epoch[name] = \
        ...
        string, s.test_accuracy[name], s.test_f1[name], s.test_loss[name] = \
            model.evaluate(test_data, test_labels, con=True, Name='Test')
        print('test  {}'.format(string))
        ...
        s.names.add(name)
        '''
        Prob_Train, HL_Train, last = model.extract(train_data, train_labels)
        Prob_Test, HL_Test, last = model.extract(test_data, test_labels)
        sio.savemat('Prob_HL_{}.mat'.format(name),
                    {'Prob_Train': Prob_Train, 'HL_Train': HL_Train, 'Prob_Test': Prob_Test, 'HL_Test': HL_Test,
                     'last_weights': last})
        '''

        ### starting the dropout code 
        if KD == True:
            print("knockdown & Stimulus Test")
            Prior_Results = model.extract(test_data, test_labels)
            sio.savemat('saved_test_data.mat', {'saved_test_data': test_data})
            sio.savemat('Prior_test_Results.mat', {'Prior__test_Results': Prior_Results})
            x = np.size(test_data, axis=0)
            y = np.size(test_data, axis=1)
            Results = np.zeros(shape=(x, 3, y, 5)).astype(np.float32)
            steps = [0, .25, .5, .75, 1]
            print("x = ", x)
            print("y = ", y)
            for i in range(y):
                if i % 50 == 0:
                    ...
                    print(i)
                # steps command if not [0-1]
                for j in range(5):
                    Updated_data = np.copy(test_data)
                    Updated_data[:, i] = steps[j]
                    temp = model.extract(Updated_data, test_labels)
                    Results[:, :, i, j] = temp
                    # Results[model.evaluate(Updated_data, test_labels, pre = True),i,j]
            # sio.savemat('Updated_data.mat', {'Updated_data':Updated_data})
            sio.savemat('Results_test.mat', {'Results_test': Results})
            sio.savemat('saved_test_labels.mat', {'saved_test_labels': test_labels})

        if KD == True:
            print("knockdown & Stimulus Validation")
            Prior_Results = model.extract(val_data, val_labels)
            sio.savemat('saved_val_data.mat', {'saved_val_data': val_data})
            sio.savemat('Prior_val_Results.mat', {'Prior_val_Results': Prior_Results})
            x = np.size(val_data, axis=0)
            y = np.size(val_data, axis=1)
            Results = np.zeros(shape=(x, 3, y, 5)).astype(np.float32)
            steps = [0, .25, .5, .75, 1]
            print("x = ", x)
            print("y = ", y)
            for i in range(y):
                ...
                # steps command if not [0-1]
                for j in range(5):
                    Updated_data = np.copy(val_data)
                    Updated_data[:, i] = steps[j]
                    temp = model.extract(Updated_data, val_labels)
                    Results[:, :, i, j] = temp
                    # Results[model.evaluate(Updated_data, test_labels, pre = True),i,j]
            # sio.savemat('Updated_data.mat', {'Updated_data':Updated_data})
            sio.savemat('Results_val.mat', {'Results_val': Results})
            sio.savemat('saved_val_labels.mat', {'saved_val_labels': val_labels})
            ##
    ...
    def show(s, fontsize=None):
        ...
        for name in sorted(s.names):
            ...
            steps *= s.params[name]['num_epochs']
            # print(s.params[name]['decay_rate']) #check
            steps = np.divide(steps, s.params[name]['decay_steps'])
            # print(s.epoch[name])
            # ax[0].plot(steps, s.train_accuracy[name], '.-', label=name)
            Tr_A.append(np.array(s.train_accuracies[name]))
            Val_A.append(np.array(s.fit_accuracies[name]))
            Tr_L.append(np.array(s.train_losses[name]))
            Val_L.append(np.array(s.fit_losses[name]))
            sio.savemat('Train_acc.mat', {'Train_acc': Tr_A})
            sio.savemat('Val_acc.mat', {'Val_acc': Val_A})
            sio.savemat('Train_loss.mat', {'Train_loss': Tr_L})
            sio.savemat('Val_loss.mat', {'Val_loss': Val_L})
            # ax[0].plot(s.epoch[name], s.fit_accuracies[name], '.-', label=name + ' Validation Data')
            # ax[0].plot(s.epoch[name], s.train_accuracies[name], '.-', label=name + ' Training Data')
            # ax[1].plot(s.epoch[name], s.fit_losses[name], '.-', label=name + ' Validation Data')
            # ax[1].plot(s.epoch[name], s.train_losses[name], '.-', label=name + ' Train Data')
        peak = np.amax(Val_A)
        Tr_A = np.vstack(Tr_A)
        Val_A = np.vstack(Val_A)
        Tr_L = np.vstack(Tr_L)
        Val_L = np.vstack(Val_L)
        end = np.amax(Val_A[:, -1])
        Tr_A = np.mean(Tr_A, axis=0)
        Val_A = np.mean(Val_A, axis=0)
        Tr_L = np.mean(Tr_L, axis=0)
        Val_L = np.mean(Val_L, axis=0)
        print(Tr_A)
        ax[0].plot(s.epoch[name], Tr_A, '.-', label='Mean Train Accuracy')
        ax[0].plot(s.epoch[name], Val_A, '.-', label='Mean Validation Accuracy')
        ax[1].plot(s.epoch[name], Tr_L, '.-', label='Mean Train Loss')
        ax[1].plot(s.epoch[name], Val_L, '.-', label='Mean Validation Loss')
        ax[0].set_xlim(0, max(s.epoch[name]))
        ax[1].set_xlim(0, max(s.epoch[name]))
