...
class base_model(object):
    ...
    def extract(self, data, labels, sess = None):
        sess = self._get_session(sess)
        size = data.shape[0]
        prob = np.empty([size,self.M[-1]])
        FCL = np.empty([size,self.M[-2]])
        for begin in range(0, size, self.batch_size):
            ...
            batch_Prob, batch_Layer = sess.run([self.op_prob, self.op_Hidlay], feed_dict)
            prob[begin:end] = batch_Prob[:end-begin,:]    
            FCL[begin:end,:] = batch_Layer[:end-begin,:]
        last = sess.run([self.last], feed_dict)    

        #feed_dict = {self.ph_data: data, self.ph_dropout: 1, self.ph_labels:labels}
        #prob, Hidlay = sess.run([self.op_prob, self.op_Hidlay], feed_dict)

        #return prob, FCL, last
        return prob
    ...
    def evaluate(self, data, labels, sess=None, con = False, Name = None, pre=False): # test = TRUE
        """
        ...
        if con is True:
            if Name is 'Test':
                sio.savemat('Confusion_Test_Pred.mat', {'predictions':predictions})
                sio.savemat('Confusion_Test_Lab.mat', {'labels':labels}) # only when testing mnist 
            elif Name is 'Train':
                sio.savemat('Confusion_Train.mat', {'predictions':predictions})
                sio.savemat('Confusion_Train_Lab.mat', {'labels':labels})
            elif Name is 'Val':
                sio.savemat('Confusion_Val.mat', {'predictions':predictions})
                sio.savemat('Confusion_Val_Lab.mat', {'labels':labels})
    ...
    def fit(self, train_data, train_labels, val_data, val_labels):
        ...
        for step in range(1, num_steps+1):
            ...
            if step % self.eval_frequency == 0 or step == num_steps:
                ...
                accuraciesV.append(accuracyV)
                lossesV.append(lossV)
                accuraciesT.append(accuracyT)
                lossesT.append(lossT)
                print('  validation {}'.format(stringV))
                print('  time: {:.0f}s (wall {:.0f}s)'.format(time.process_time()-t_process, time.time()-t_wall))
                epochs.append(epoch)
                # Summaries for TensorBoard.
                summary = tf.Summary()
                summary.ParseFromString(sess.run(self.op_summary, feed_dict))
                summary.value.add(tag='validation/accuracy', simple_value=accuracyV)
                summary.value.add(tag='validation/f1', simple_value=f1)
                summary.value.add(tag='validation/loss', simple_value=lossV)
        ...
        stringV, accuracyV, f1, lossV = self.evaluate(val_data, val_labels, sess)
        print('validation accuracy: peak = {:.2f}, mean = {:.2f}'.format(max(accuraciesV), np.mean(accuraciesV[-10:])))
        writer.close()

        sess.close()
        
        t_step = (time.time() - t_wall) / num_steps
        return accuraciesV, lossesV, t_step, accuraciesT, lossesT, epochs
    ...
    def build_graph(self, M_0):
        ...
        with self.graph.as_default():
            ...
            # Model.
            op_logits, op_HL = self.inference(self.ph_data, self.ph_dropout)
            self.op_loss, self.op_loss_average = self.loss(op_logits, self.ph_labels, self.regularization)
            self.op_train = self.training(self.op_loss, self.learning_rate,
                    self.decay_steps, self.decay_rate, self.momentum)

            self.op_prob = self.probabilities(op_logits)
            self.op_prediction = self.prediction(self.op_prob)
            self.op_Hidlay = self.HL(op_HL)
            ...
            self.last = self.graph.get_tensor_by_name("logits/weights:0")
        self.graph.finalize()
    ...
    def inference(self, data, dropout):
        ...
        logits, HL = self._inference(data, dropout)
        return logits, HL
    
    def HL(self,HiddenLay):
...
class cgcnn(base_model):
    ...
    def chebyshev5(self, x, L, Fout, K):
        ...
        print(L)
        print(x0.shape)
        x0 = tf.sparse_tensor_dense_matmul(L, x0)  #dot product of sparse tensor and dense
    ...
    def _inference(self, x, dropout):
        ...
        with tf.variable_scope('logits'):
            y = self.fc(x, self.M[-1], relu=False)
        return y,x
